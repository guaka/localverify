...
**Store selection:** If the user names a store (a store is a standalone OpenSpec repo registered on this machine) or the work lives in one, run `openspec store list --json` to discover registered store ids, then pass `--store <id>` on the commands that read or write specs and changes (`new change`, `status`, `instructions`, `list`, `show`, `validate`, `archive`, `doctor`, `context`, `schemas`, `view`). Once selected, treat `--store <id>` as sticky for the rest of the workflow. Every unscoped example of those commands below is shorthand: before running it, append the flag. For example, run `npm run spec -- status --change "<name>" --json --store "<id>"`, not the unscoped form shown below. Other commands do not take the flag. Hints printed by commands already carry the flag; keep it on follow-ups. Without a store, commands act on the nearest local `openspec/` root.
...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   - Asks to "show workflows" or asks "what workflows" exist → resolve the authoritative root by running `openspec context --json` from the current working directory. If the user explicitly selected a registered store, use `openspec context --json --store "<store-id>"`. Then run `npm run spec -- schemas --json` with its working directory set to the returned `root.path` and let them choose. This preserves roots selected by a local `store:` pointer or the global `defaultStore`; when a registered store was explicitly selected, append `--store "<store-id>"` to `npm run spec -- schemas --json` as well. If context reports only `no_openspec_root`, run `npm run spec -- schemas --json` from the current working directory instead. Do not use this fallback for invalid or unavailable stores.
...
3. **Create the change directory**
   ...
   ```bash
   npm run spec -- new change "<name>"
   ```

   Using an explicitly requested schema:
   ```bash
   npm run spec -- new change "<name>" --schema "<schema-name>"
   ```
...
5. **Create every artifact in the required set**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      - Get instructions:
        ```bash
        npm run spec -- instructions <artifact-id> --change "<name>" --json
        ```
   ...
   b. **Continue until every artifact in the required set exists (not just `apply.requires`)**
      - After creating each artifact, re-run `npm run spec -- status --change "<name>" --json`
...
6. **Show final status**
   ```bash
   npm run spec -- status --change "<name>"
   ```
...
- Follow the `instruction` field from `npm run spec -- instructions` for each artifact type - it is the authoritative guidance, even for familiar artifact names
